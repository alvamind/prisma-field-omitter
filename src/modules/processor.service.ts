...
import { Config, HideRule } from '../types';
...
export const processorService = Alvamind({ name: 'processor.service' })
  .use(loggerService)
  .derive(({ loggerService: { info, warn } }) => ({
    processorService: {
      ...
      applyRule: (content: string, rule: HideRule, config: Config, stats: ProcessingStats) => {
      ...
      processFile: async (
        ...
        try {
          ...
          for (const rule of config.hide) {
            content = self.applyRule(content, rule, config, stats);
          }
          ...
          if (content !== originalContent) {
            ...
            info(`Processed: ${path.basename(filePath)}`);
          }
        } catch (error) {
          warn(`Error processing file ${filePath}: ${(error as any).message}`);
        }
      }
    }
  }));
